...
def DFColumnTranslate_baidu(DFColumn, appid, appkey, from_lang: str = 'auto', to_lang: str = 'en'):
    """
...
def DFColumnTranslate_MS(DFColumn, ms_subscribeKey, dynamic_dict: dict = False, from_lang: str = None,
                         to_lang: str = 'en'):
    """
    ...
    Translate, response = MStranslation_dynamicDictionary_API(DFColumn_list, dynamic_dict=dynamic_dict,
                                                              lang_in=from_lang,
                                                              lang_out=to_lang, subscription_key=ms_subscribeKey)
    if 'error' in response:
        return response

    Column_name = DFColumn.keys()[0]
    if isinstance(Column_name, str):
        Column_name, response = MStranslation_dynamicDictionary_API(Column_name, dynamic_dict=dynamic_dict,
                                                                    lang_in=from_lang,
                                                                    lang_out=to_lang, subscription_key=ms_subscribeKey)
        if 'error' in response:
            return response
...
def ExcelColumnTranslate(translate_engine: translate_engine = 1, ms_subscribeKey=None, baidu_appid=None,
                         baidu_appkey=None, from_lang: Union[str, None] = 'auto', to_lang='en', ExcelPath: str = None,
                         readSheet=None, readHeader: int = None, readStartRow: Union[int, None] = 1,
                         readCol: Union[int, list[str]] = None, nrows: int = None,
                         write2Sheet: str = None, write2Row: int = 2, write2Col: int = 1):
    """
    ...
    if translate_engine == translate_engine.baidu:
        TransData = DFColumnTranslate_baidu(ExcelData, baidu_appid, baidu_appkey, from_lang, to_lang)
    elif translate_engine == translate_engine.microsoft:
        TransData = DFColumnTranslate_MS(ExcelData, ms_subscribeKey=ms_subscribeKey, from_lang=from_lang,
                                         to_lang=to_lang)
    ...
    Write2Excel(TransData, ExcelPath, write2Sheet, write2Row, write2Col - 1, writeheader)  # -2 因为index与行号差1
    print(f"翻译并写入完成! 请查看{ExcelPath}.{write2Sheet}")


if __name__ == "__main__":
    # 读取需要翻译的Excel文件,装入DataFrame
    from_lang = 'auto'  # 百度: 'auto', 微软: None; 自动辨识语言;当双语混杂时,需要指定语言,否则会有漏译情况;
    to_lang = 'en'  # 百度:( 中文: zh;文言文: wyw;日本: jp; --> 伊朗语: ir; 波斯语);微软: "zh-Hans"中文简体
    ExcelPath = r"E:/Working Documents/Eastcom/Russia/Igor/专网/LeoTelecom/发货测试验收/BOM Packing List.xlsx"
    readSheet = 'Sheet1'
    readHeader = 1
    readStartRow = None
    ...
    baidu_appid, baidu_appkey = config_read(config_path, section="baidu_OpenAPI", option1='appid', option2='appkey')
    ms_subscribeKey = config_read(MSconfig_path, section="MS_translation", option1="subscription_key", option2=None)

    # 百度翻译引擎:
    ExcelColumnTranslate(translate_engine=translate_engine.baidu, baidu_appid=baidu_appid, baidu_appkey=baidu_appkey,
                         from_lang=from_lang, to_lang=to_lang, ExcelPath=ExcelPath,
    ...
    ExcelColumnTranslate(translate_engine=translate_engine.microsoft, ms_subscribeKey=ms_subscribeKey,
                         ...
                         baidu_appkey=baidu_appkey, from_lang=None, to_lang=to_lang,
                         ExcelPath=ExcelPath,
                         readSheet=readSheet, readHeader=readHeader, readStartRow=readStartRow, readCol=readCol,
                         nrows=nrows,
